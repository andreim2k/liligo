#!/usr/bin/env python3
"""
KeyBridge Menu Bar App - Working version with icon and clipboard functionality
"""
...
class KeyBridgeApp(rumps.App):
    def __init__(self):
        # Try to load custom icon first, fallback to emoji
        try:
            import os
            icon_path = os.path.join(os.path.dirname(__file__), "KeyBridge.icns")
            if os.path.exists(icon_path):
                super(KeyBridgeApp, self).__init__("KeyBridge", "KeyBridge", icon=icon_path, template=True)
                print("✅ Loaded custom icon")
            else:
    ...
    async def _send_clipboard_flow(self, text):
        ...
        try:
            ...
            rumps.notification("KeyBridge", "Success!", f"Sent {len(text)} characters to KeyBridge")
